...
/**
 * Check for Avanza duplicates
 *
 * Usage:
 *   tsx backend/scripts/check-avanza-duplicates.ts
 */
...
const logger = new Logger('CheckAvanzaDuplicates');
...
async function checkAvanzaDuplicates() {
    logger.info('🔍 Checking for parties with "avanza" in name...\n');
    ...
    if (!parties || parties.length === 0) {
        logger.info('No parties found with "avanza" in name');
        return;
    }
